...
class KANRegressor(nn.Module):
    ...
    def __init__(self, in_dim: int, num_targets: int, n_basis: int = 8):
        super().__init__()
        self.l1 = KANLayer(in_dim, 64, n_basis=n_basis)
        self.act1 = nn.GELU()
        self.l2 = KANLayer(64, 32, n_basis=n_basis)
        self.act2 = nn.GELU()
        self.out = nn.Linear(32, num_targets)
...
class SMARTNIRRegressor(nn.Module):
    def __init__(self, cfg: SmartNIRRegressorConfig):
        super().__init__()
        self.cfg = cfg
    ...
    def forward(self, r: torch.Tensor) -> torch.Tensor:
        ...
        cls = z[:, 0, :]                   # (B, D)
        preds = self.head(cls)            # (B, num_classes)
        return preds
